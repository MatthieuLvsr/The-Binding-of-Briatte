...
void addRoom(Floor* floor, int y, int x, int id){
    // srand(time(NULL));
...
Floor* newFloor(int stage){
    ...
    addRoom(floor,floor->curY,floor->curX,0);
    fillFloor(floor);
    return floor;
}
...
void nextStep(Floor* floor, PLAYER player, SHOOT* shoots){
    RoomItem* room = getRoomByCoord(floor->list,player.roomX,player.roomY);
    displayRoom(room->map->room,room->map->height,room->map->width,player, shoots);
}